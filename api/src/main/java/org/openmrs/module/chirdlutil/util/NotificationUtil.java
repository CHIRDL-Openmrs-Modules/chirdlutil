...
{	
	...
	public static void sendSupportEmailNotification(String message) {
		...
		try {
			...
			if (canSendMessage(message)) {
				...
				String salutation = Context.getAdministrationService().getGlobalProperty(ChirdlUtilConstants.GLOBAL_PROP_SUPPORT_EMAIL_SALUTATION);
    ...
    /**
     * Checks if message has already been sent within a specified time frame.
     * This prevents sending multiple instances of the same message too frequently. 
     * @param message
     * @returns false if this message has been sent already within the specified time threshold.
     */
    private static boolean canSendMessage(String message) {
    	Long time = null;
    	String threshold = Context.getAdministrationService().getGlobalProperty(ChirdlUtilConstants.GLOBAL_PROP_MESSAGE_FREQUENCY_THRESHOLD);
		...
		//If the key does not exist, it is added with value of currTime, and the return value will be currTime.
		//If the key does exist, the return value will be the value of the existing key.
		//Send the message if the value was just added
		if (( time =  messageToTimeMap.computeIfAbsent(messageHash, k -> currTime)) == currTime ) {
			return true;
		}
		
		//Check if time has exceeded the threshold wait time
